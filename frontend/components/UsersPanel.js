...
export default function UsersPanel() {
  ...
  return (
    <section className="space-y-6 max-w-3xl">
      <form onSubmit={createUser} className="bg-surface border border-border rounded-xl p-4 shadow-sm grid grid-cols-5 gap-2 items-end">
        <div>
              ...
              <tr key={u.id} className="border-t border-border align-top">
                <td className="px-4.5 py-3">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-full bg-accentsoft text-accent text-[10px] font-bold flex items-center justify-center">
                      {initials(u.name)}
                    </span>
                    <div>
                      <div className="font-medium text-ink">{u.name}</div>
                      <div className="text-xs text-inkfaint">{u.email}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4.5 py-3 text-inksoft">{ROLE_LABEL[u.role]}</td>
                <td className="px-4.5 py-3">
