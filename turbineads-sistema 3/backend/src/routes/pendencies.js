...
router.patch("/:id", requireRole("SOCIO", "GESTOR"), async (req, res) => {
  if (req.user.role === "GESTOR") {
    const owned = await prisma.pendency.findFirst({
      where: { id: req.params.id, client: { gestorId: req.user.id } },
    });
    if (!owned) return res.status(403).json({ error: "Você não tem acesso a essa pendência." });
  }
